...
type Property = {
  ...
  featured?: boolean;
  rooms?: Room[];
  // optional frontend-only external website for partner/static properties
  website?: string;
  safari_rooms?: Room[];
};
...
export default function PropertyCard({ property }: PropertyCardProps) {
  ...
  const renderMedia = () => {
    ...
    if (!firstMedia || typeof firstMedia !== 'string') {
      ...
      return {
        mediaContent: (
          <div className="w-full h-48 bg-[#CFE7F8] flex items-center justify-center">
            <span className="text-[#AAA59F]">No image available</span>
          </div>
        ),
        isVideo: false
      };
    ...
    if (videoCheck) {
      ...
      if (lowerUrl.includes('youtube.com') || lowerUrl.includes('youtu.be')) {
        mediaContent = (
          <div className="relative w-full aspect-video overflow-hidden bg-[#CFE7F8]">
        ...
        mediaContent = (
          <div className="relative w-full aspect-video overflow-hidden bg-[#CFE7F8]">
            ...
            <div className="w-full h-full bg-gradient-to-br from-[#CFE7F8] to-[#92AAD1]/30 flex items-center justify-center">
              <div className="text-center text-[#AAA59F]">
            ...
            >
              <div className="bg-[#749DD0]/90 rounded-full p-4 hover:bg-[#48547C] transition-colors shadow-lg">
                <Play className="h-8 w-8 text-white" />
              </div>
            </div>
          </div>
        ...
        mediaContent = (
          <div className="relative w-full aspect-video overflow-hidden bg-[#CFE7F8]">
            ...
            <div className="video-fallback absolute inset-0 items-center justify-center bg-[#CFE7F8] hidden">
              <div className="text-center text-[#AAA59F]">
                <Play className="h-12 w-12 mx-auto mb-2" />
      ...
      mediaContent = (
        <img
          ...
          onError={(e) => {
            ...
            e.currentTarget.classList.add('bg-[#CFE7F8]');
          }}
        />
      );
    }
  ...
  return (
    <Card className="overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 bg-white border border-[#92AAD1]/20 rounded-2xl group">
      <div className="relative overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2">
          {videoCheck && (
            <Badge className="bg-[#749DD0] hover:bg-[#48547C] text-white text-xs rounded-full px-3 py-1 shadow-lg">
              VIDEO
            </Badge>
          )}
          {propertyFeatured && (
            <Badge className="bg-gradient-to-r from-amber-400 to-orange-500 text-white text-xs rounded-full px-3 py-1 shadow-lg">
              Featured
            </Badge>
          )}
        </div>
        ...
        {/* Availability Badges */}
        {fullyBooked && (
          <Badge className="absolute top-3 right-3 bg-red-500 text-white rounded-full px-3 py-1 shadow-lg">
            Fully Booked
          </Badge>
        )}
        {!fullyBooked && availableRooms < totalRooms && totalRooms > 0 && (
          <Badge className="absolute top-3 right-3 bg-amber-500 text-white rounded-full px-3 py-1 shadow-lg">
            Limited
          </Badge>
        )}
      ...
      <CardContent className="p-5 bg-white">
        <div className="space-y-3">
          <div>
            <h3 className="text-lg font-semibold text-[#33343B] hover:text-[#749DD0] transition-colors duration-300 line-clamp-1">{propertyName}</h3>
            <div className="flex items-center text-[#AAA59F] text-sm mt-1">
          ...
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center">
              ...
              <span className="font-medium text-[#33343B]">{propertyRating}</span>
              <span className="text-[#AAA59F] ml-1">({propertyReviews})</span>
            </div>
            {rooms.length > 0 && (
              <div className="flex items-center text-[#AAA59F]">
                <Users className="h-4 w-4 mr-1" />
                {Math.max(...rooms.map(r => (r.maxGuests || r.max_guests || 0)))} guests
              </div>
            )}
          </div>
            ...
            {fullyBooked ? (
              <div className="bg-red-50 border border-red-200 rounded-xl p-3">
                <div className="flex items-center text-red-800">
                  ...
                  <span className="font-medium text-sm">Fully Booked</span>
                </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
                ...
                {earliestAvailableDate && (
                  <div className="flex items-center text-amber-600 text-xs mt-1">
                    <Clock className="h-3 w-3 mr-1" />
                    <span>More rooms from {earliestAvailableDate.toLocaleDateString()}</span>
                  </div>
                )}
              </div>
            ) : (
              <div className="bg-[#CFE7F8]/30 border border-[#749DD0]/30 rounded-xl p-3">
                <div className="flex items-center text-[#33343B]">
                  <Calendar className="h-4 w-4 mr-2 text-[#749DD0]" />
                  <span className="font-medium text-sm">Available Now</span>
                </div>
                <span className="text-[#AAA59F] text-xs">
                  All {totalRooms} rooms available
                </span>
              </div>
            )}
          </div>

          <p className="text-[#AAA59F] text-sm line-clamp-2">{propertyDescription}</p>

          <div className="flex items-center justify-between pt-3 border-t border-[#92AAD1]/20">
            <div>
              <span className="text-xl font-bold text-[#749DD0]">${propertyPrice}</span>
              <span className="text-[#AAA59F] text-xs ml-1">/ night</span>
            </div>
            <div className="flex gap-2">
              {(property.website && typeof property.website === 'string' && property.website.trim() !== '') ? (
                <a href={property.website} target="_blank" rel="noopener noreferrer">
                  <Button 
                    className="bg-white hover:bg-[#CFE7F8] text-[#33343B] border border-[#92AAD1]/30 hover:border-[#749DD0] transition-all duration-300 rounded-full text-xs px-4"
                    size="sm"
                  >
                    View
                  </Button>
              ...
              <Link to={`/book?property=${propertySlug}`}>
                <Button
                  className="bg-[#749DD0] hover:bg-[#48547C] text-white transition-all duration-300 rounded-full text-xs px-4"
                  disabled={fullyBooked}
                  size="sm"
                >
                  {fullyBooked ? 'Booked' : 'Book'}
                </Button>
              </Link>
            </div>
          </div>
